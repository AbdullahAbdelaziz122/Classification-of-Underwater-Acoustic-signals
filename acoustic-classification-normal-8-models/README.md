...
To add **Torpedo** and **Submarine** data, set the paths in Section 2:

```python
TORPEDO_DIR   = '/kaggle/input/your-torpedo-dataset'
SUBMARINE_DIR = '/kaggle/input/your-submarine-dataset'
```

If either path is `None`, that class is skipped gracefully and training continues without it.
...
This produces spectrograms of shape `(1, 64, ~84)` — 64 mel bins × ~84 time frames per 2‑second clip.
...
The final test evaluation was conducted on a held‑out test set using GPU inference on the training machine. The table below reports per‑model classification metrics together with average per‑sample inference latency (ms). These latency figures serve as a baseline for comparison with Raspberry Pi edge deployment results (to be added after benchmarking on the Pi).

| Model | Params (M) | Accuracy | Precision | Recall | F1 | Latency (ms) |
|-------|------------|----------|-----------|--------|----|--------------|
